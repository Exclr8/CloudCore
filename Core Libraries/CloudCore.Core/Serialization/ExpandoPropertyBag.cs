...
namespace CloudCore.Core.Serialization
{
    ...
    /// </summary>
    [XmlRoot("properties")]
    public class ExpandoPropertyBag : ExpandoPropertyBag<object>
    {
        /// <summary>
        ...
        /// <returns></returns>
        public new static ExpandoPropertyBag CreateFromXml(string xml)
        {
            var bag = new ExpandoPropertyBag();
            bag.FromXml(xml);
            return bag;
        }
    }
    ...
    [XmlRoot("properties")]
    public class ExpandoPropertyBag<TValue> : Dictionary<string, TValue>, IXmlSerializable
    {
        /// <summary>
        ...
        public void WriteXml(System.Xml.XmlWriter writer)
        {
            foreach (string key in this.Keys)
            {
                ...
                string xmlType = GenericUtils.MapTypeToXmlType(type);
                bool isCustom = false;
            ...
            while (reader.Read())
            {
                if (reader.NodeType == XmlNodeType.Element && reader.Name == "key")
                {
                    ...
                    else if (xmlType.StartsWith("___"))
                    {
                        ...
                        Type type = GenericUtils.GetTypeFromName(xmlType.Substring(3));
                        XmlSerializer ser = new XmlSerializer(type);
                        value = (TValue)ser.Deserialize(reader);
                    }
                    else
                        value = (TValue)reader.ReadElementContentAs(GenericUtils.MapXmlTypeToType(xmlType), null);
        ...
        {
            string xml = null;
            Utilities.Serialization.SerializeObject(this, out xml);
            return xml;
        }
        ...
        public bool FromXml(string xml)
        {
            ...
            var result = Utilities.Serialization.DeSerializeObject(xml,
                                                 this.GetType()) as ExpandoPropertyBag<TValue>;
            if (result != null)
            {
        ...
        /// <param name="xml"></param>
        /// <returns></returns>
        public static ExpandoPropertyBag<TValue> CreateFromXml(string xml)
        {
            var bag = new ExpandoPropertyBag<TValue>();
            bag.FromXml(xml);
            return bag;
        }
    }
