...
namespace CloudCore.Web.Core.Controls.Grid.Components
{
	...
	public class ColumnBuilder<T> : IList<GridColumn<T>> where T : class 
	{
		...
		{
            return For(propertySpecifier, propertySpecifier.Compile());
		}
        ...
        /// </summary>
        /// <param name="propertySpecifier">Lambda that specifies the property for which a column should be constructed</param>
        public IGridColumn<T> For(Expression<Func<T, object>> propertySpecifier, Func<T, object> displayTextSpecifier)
        {
            ...
            var column = new GridColumn<T>(displayTextSpecifier, inferredName, propertyType);
		...
		public static Type GetTypeFromMemberInfo<TMember>(MemberInfo member, Func<TMember, Type> func) where TMember : MemberInfo 
		{
		...
		public static Expression RemoveUnary(Expression body)
		{
